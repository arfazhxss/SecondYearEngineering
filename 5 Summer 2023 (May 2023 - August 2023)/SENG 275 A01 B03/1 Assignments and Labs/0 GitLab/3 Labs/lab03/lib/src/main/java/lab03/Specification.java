...
public class Specification {
    ...
    public static boolean messageIsValidCorrected(String input, boolean motorcycle) {
        if (input.length() < 2 || input.length() > 7 || !motorcycle) return false;

        // Count the number of letters and numbers (excluding spaces and hyphens)
        int count = 0;
        ...
        for (int i = 0; i < input.length(); i++) {
            ...
            if (currentChar == ' ' || currentChar == '-') {
                if (i == 0 || i == input.length() - 1) {
                    return false; // Invalid placement at the beginning or end
                }
                ...
                if (!Character.isLetterOrDigit(prevChar) || !Character.isLetterOrDigit(nextChar)) {
                    return false; // Invalid placement without a letter or number on both sides
                }
            }
        }
